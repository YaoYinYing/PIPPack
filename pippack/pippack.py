...
def replace_protein_sequence(protein, protein_name, new_seqs: list[list[str]]):
    proteins = []

    for i, seqs in enumerate(new_seqs):
        # Verify that the lengths match.
        print(f"checking {i}: seqs: {seqs}")
        if len(np.unique(protein["chain_index"])) != len(seqs):
            raise ValueError(
                f"Length of number of chains in the new sequence ({len(seqs)}) does not match the number of chains in the protein ({len(np.unique(protein['chain_index']))})."
            )
        else:
            for chain in np.unique(protein["chain_index"]):
                if len(seqs[chain]) != len(
                    np.where(protein["chain_index"] == chain)[0]
                ):
                    raise ValueError(
                        f"Length of sequence for chain {chain} ({len(seqs[chain])}) does not match the number of residues in chain {chain} ({len(np.where(protein['chain_index'] == chain)[0])})."
                    )
...
def sample_epoch(
    ...
    if isinstance(model, list):
        ...
        chi_bin_one_hot = torch.nn.functional.one_hot(
            chi_bin, num_classes=_model.n_chi_bins + 1
        )
        ...
        if chi_bin_offset is not None:
            ...
            bin_sample_update = (2 * torch.pi / _model.n_chi_bins) * torch.rand(
...
class PIPPack:
    ...
    def _initialize_with_a_model(
        ...
        with open(
            os.path.join(self.weights_path, f"{self.model_names}_config.pickle"), "rb"
        ) as f:
        ...
        self.device = torch.device(
            "cuda:0" if (torch.cuda.is_available() and not self.force_cpu) else "cpu"
        )
        print(f"will run on {self.device}")

        # Load model with same config
        self.loaded_models: torch.nn.Module = hydra.utils.instantiate(
            self.exp_cfg.model
        ).to(self.device)
    ...
    def _initialize_with_ensemble(self):
        ...
        for model_name in self.model_names:
            # Get the config used when running experiment
            with open(
                os.path.join(self.weights_path, f"{model_name}_config.pickle"), "rb"
            ) as f:
            ...
            # Load model with same config
            model: torch.nn.Module = hydra.utils.instantiate(self.exp_cfg.model).to(
                self.device
            )
    ...
    def _process_a_batch(self, batch: list, output_dir: str):
        ...
        sample_results = sample_epoch(
            ...
            resample=self.use_resample,
            resample_args=self.resample_args,
        )
